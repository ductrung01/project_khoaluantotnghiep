...
const CreateProductWithVariants: React.FC = () => {
    ...
    const handleSubmit = () => {
        if (!name || !description || !price || !selectedBrand || selectedSizes.length === 0) {
            toast.error('Vui lòng nhập đầy đủ thông tin sản phẩm và Chọn năm xuất hoặc tái bản bản', { autoClose: 3000 });
            return;
        }
    ...
    return (
        <Card sx={{ maxWidth: 800, margin: 'auto', padding: 3, marginTop: 5 }}>
            ...
            <Box display="flex" flexDirection="column" gap={2}>
                ...
                <FormControl fullWidth>
                    <InputLabel id="volume-select-label">Dung tích</InputLabel>
                    <Select
                    ...
                    >
                        ...
                        {volumes.map((volume) => (
                            <MenuItem key={volume} value={volume}>{volume} ml</MenuItem>
                        ))}
                    </Select>
                </FormControl>
                <FormControl fullWidth>
